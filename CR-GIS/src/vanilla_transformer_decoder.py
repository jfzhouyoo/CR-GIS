...
def _build_decoder(opt, dictionary, embedding=None, padding_idx=None,
    ...
    return TransformerDecoder(
        n_heads=opt['decoder_num_head'],
        n_layers=opt['decoder_num_layers'],
        embedding_size=opt['encoder_hidden_size'],
        ffn_size=opt['ffn_size'],
        vocabulary_size=len(dictionary),
        word_embedding_dim=opt["encoder_token_emb_dim"],
        embedding=embedding,
        dropout=opt['encoder_dropout'],
        attention_dropout=opt['decoder_attention_dropout'],
        relu_dropout=opt['decoder_relu_dropout'],
        padding_idx=padding_idx,
        learn_positional_embeddings=opt.get('learn_positional_embeddings', False),
        embeddings_scale=opt['embeddings_scale'],
        n_positions=n_positions,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_output, encoder_mask, user_embed):
        decoder_mask = self._create_selfattn_mask(x)
        user_embed = user_embed.unsqueeze(1).repeat(1,x.shape[1],1)
        # first self attn
        residual = x
        # don't peak into the future!
        x = self.self_attention(query=x, mask=decoder_mask)
        x = self.dropout(x)  # --dropout
        x = x + residual + user_embed 
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.fc = nn.Linear(word_embedding_dim, embedding_size)
    ...
    def forward(self, input, encoder_state, user_embed, incr_state=None):
        ...
        tensor = self.embeddings(input)
        if tensor.size(-1) != self.embedding_size:
        ...
        for layer in self.layers:
            tensor = layer(tensor, encoder_output, encoder_mask, user_embed)
